...
cleanup_resources() {
  local exit_code=$?
  echo "Cleaning up..."
  if [[ -n "${COMPILE_HEARTBEAT_PID}" ]]; then
    kill "${COMPILE_HEARTBEAT_PID}" 2>/dev/null || true
  fi
  cleanup_argocd_e2e "${ARGOCD_NAMESPACE:-argocd-e2e}"
  exit "$exit_code"
}
...
if [[ -n "${PREBUILT_DIR}" ]]; then
  ...
  if [[ -f "${PREBUILT_DIR}/e2e.test" && -f "${PREBUILT_DIR}/dist/argocd" ]]; then
    ...
    if [[ ("${BINARY_ARCH}" == "x86-64" && "${TARGET_ARCH}" == "amd64") || \
      ...
      # Override pre-built argocd CLI with release-candidate binary
      if [[ -n "${ARGOCD_SERVER_IMAGE:-}" ]]; then
        echo "Extracting release-candidate argocd CLI from ${ARGOCD_SERVER_IMAGE}..."
        ...
        for bin_path in /usr/local/bin/argocd /usr/bin/argocd; do
          RC_TMP=$(mktemp -d)
          if DOCKER_CONFIG="${EXTRACT_AUTH_DIR}" oc image extract "${ARGOCD_SERVER_IMAGE}" \
              --filter-by-os="linux/${TARGET_ARCH}" \
              --path "${bin_path}:${RC_TMP}/" --confirm 2>&1; then
            if [[ -f "${RC_TMP}/argocd" ]]; then
              chmod +x "${RC_TMP}/argocd"
              if "${RC_TMP}/argocd" version --client --short 2>/dev/null; then
                cp "${RC_TMP}/argocd" "${ARGO_CD_DIR}/dist/argocd"
                echo "Pre-built argocd CLI overridden with release-candidate"
                rm -rf "${RC_TMP}"
                break
              fi
            fi
...
if [[ ! -d "${ARGO_CD_DIR}" ]]; then
  ...
  # Try to use release-candidate argocd CLI from deployed image
  RC_ARGOCD=false
  if [[ -n "${ARGOCD_SERVER_IMAGE:-}" ]]; then
    echo "Extracting argocd CLI from release-candidate image: ${ARGOCD_SERVER_IMAGE}"
    EXTRACT_AUTH_DIR=$(mktemp -d)
    oc get secret pull-secret -n openshift-config \
      -o jsonpath='{.data.\.dockerconfigjson}' | \
      base64 -d > "${EXTRACT_AUTH_DIR}/config.json" 2>/dev/null || true
    ...
    for bin_path in /usr/local/bin/argocd /usr/bin/argocd; do
      if DOCKER_CONFIG="${EXTRACT_AUTH_DIR}" oc image extract "${ARGOCD_SERVER_IMAGE}" \
          --filter-by-os="linux/${TARGET_ARCH}" \
          --path "${bin_path}:${ARGO_CD_DIR}/dist/" --confirm 2>&1; then
        if [[ -f "${ARGO_CD_DIR}/dist/argocd" ]]; then
          chmod +x "${ARGO_CD_DIR}/dist/argocd"
          if "${ARGO_CD_DIR}/dist/argocd" version --client --short 2>/dev/null; then
            RC_ARGOCD=true
            echo "Using release-candidate argocd CLI"
            break
          else
            rm -f "${ARGO_CD_DIR}/dist/argocd"
          fi
        fi
      fi
    done
  ...
  if [[ "${RC_ARGOCD}" != "true" ]]; then
    echo "Falling back to building argocd CLI from source..."
