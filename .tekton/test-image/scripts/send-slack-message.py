...
def setup_oras_auth(creds_path="/quay-credentials/.dockerconfigjson"):
    """Setup oras authentication by configuring DOCKER_CONFIG env var."""
    if not creds_path or not os.path.isfile(creds_path):
        return False

    tmpdir = tempfile.mkdtemp()
    shutil.copy2(creds_path, os.path.join(tmpdir, "config.json"))
...
def build_blocks(
    ...
    # Add test summary from combined logs artifact
    if quay_repo and pipeline_run_name:
        summary = get_combined_logs_summary(quay_repo, pipeline_run_name)
        if summary:
            # Check if it's test results or warnings
            if summary.startswith('Tests:'):
                # Parse test counts and format with emojis
                blocks.append(
                    {
                        "type": "section",
                        "text": {
